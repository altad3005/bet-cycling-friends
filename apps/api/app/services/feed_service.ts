    ...
    if (raceIds.length > 0 && memberIds.length > 0) {
      ...
      const [classicBets, gtBets]: any[][] = await Promise.all([
        db
          ...
          .select(
            'bets_classic.user_id',
            'bets_classic.race_id',
            'bets_classic.placed_at',
            'users.pseudo',
            'users.icon'
          ),
        db
          ...
          .select(
            'bets_grand_tour.user_id',
            'bets_grand_tour.race_id',
            'bets_grand_tour.placed_at',
            'users.pseudo',
            'users.icon'
          ),
      ])
      ...
      const raceResultMap = new Map<
        string,
        { publishedAt: string; pseudo: string; icon: string; points: number }
      >()
      for (const s of scores) {
        if (!raceResultMap.has(s.race_id)) {
          raceResultMap.set(s.race_id, {
