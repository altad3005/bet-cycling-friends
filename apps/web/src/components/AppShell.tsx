...
export default function AppShell({
  ...
  const { data: leagueStandings } = useQuery({
    ...
    queryFn: () =>
      standingsApi.league(activeLeague!.id).then((r) => r.data.data.standings),
      ...
      >
        ...
        {activeLeague && (
          ...
          >
            ...
            <svg
              className={`league-switcher-chevron${showDropdown ? " open" : ""}`}
              viewBox="0 0 24 24"
              width="12"
              height="12"
              fill="none"
        ...
        {!activeLeague && myLeagues && myLeagues.length > 0 && (
          ...
          >
            <span
              className="league-switcher-name"
              style={{ color: "rgba(240,237,232,0.4)", fontStyle: "italic" }}
            >
              Choisir une ligue
            </span>
            <svg
          ...
          <div className="sidebar-inline-form">
            ...
            <input
              ...
              onKeyDown={(e) =>
                e.key === "Enter" &&
                leagueName &&
                createMutation.mutate(leagueName, {
                  onSuccess: () => {
                    setShowCreateForm(false);
                    setLeagueName("");
                  },
            ...
            <div className="sidebar-inline-actions">
              ...
              <button
                className="sidebar-inline-btn ghost"
                onClick={() => {
                  setShowCreateForm(false);
                  setLeagueName("");
                }}
        ...
        {showJoinForm && (
          <div className="sidebar-inline-form">
            ...
            {joinMutation.isError && (
              <div className="sidebar-inline-error">
        ...
        >
          ...
          {myStanding && (
            <div className="me-rank">
              {myStanding.rank}e ·{" "}
              {myStanding.totalPoints.toLocaleString("fr-FR")} pts
            </div>
          )}
        </div>
        <button
