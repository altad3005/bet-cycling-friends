...
function iconColor(type: FeedEvent['type']) {
...
function EventRow({ event }: { event: FeedEvent }) {
  ...
  const clickable = event.userId && (event.type === 'bet_placed' || event.type === 'member_joined')

  return (
    <div
      style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '0.75rem 0', borderTop: '0.5px solid rgba(255,255,255,0.05)', cursor: clickable ? 'pointer' : 'default' }}
      onClick={() => clickable && navigate(`/members/${event.userId}`)}
    >
      <div style={{ width: 32, height: 32, borderRadius: 9, background: ic.bg, color: ic.color, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
        <EventIcon type={event.type} />
      </div>
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontSize: 13, color: '#f0ede8', fontWeight: 500, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
          {title}
        </div>
        {subtitle && (
          <div style={{ fontSize: 12, color: 'rgba(240,237,232,0.4)', marginTop: 2, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
            {subtitle}
          </div>
        )}
      </div>
...
export default function ActivityFeedPanel({ events, hasMore }: Props) {
  const navigate = useNavigate()
